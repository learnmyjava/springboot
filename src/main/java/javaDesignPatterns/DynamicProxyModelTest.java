 ...
 * @version:
 * 结构类模式
 *  代理模式 为其他对象提供一个代理以控制被代理类的访问。 代理类主要负责为被代理类预处理消息，过滤消息，传递消息给被代理类，
 *  代理类不实现具体业务逻辑，利用委托类来完成服务，之后还能进行消息的后置处理。
 *           
 * spring AOP 动态代理分两种:
 *   JDK动态代理 :代理类需要实现一个接口，核心类InvocationHandler,Proxy,JDK代理限制了只能为接口创建代理实例，对于没有通过接口定义的业务类，就无法使用JDK。所以它是面向接口的代理
 *   CGLIB动态代理：CGLIB可以为一个类创建一个子类，在子类中采用拦截的技术拦截所以父类方法的调用并顺序织入横切逻辑,若这个类被fianl 修饰，则无法实现代理，
